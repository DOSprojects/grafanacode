...
'''
    grafanacode:
    class for generating news panels
'''
...
#******************************************************************************
@define(slots=False)
class PanelNews(panel_base.PanelBase):
    '''
    ...
    loglevel           : int = 15
    # attributes
    showimage          = field( default=False,
                                validator=instance_of(bool))
    feedurl            = field( default='https://grafana.com/blog/news.xml',
    ...
    def __attrs_post_init__(self):
        ...
        '''
        print (f'  > Init Panel: {self.title} - type: PanelNews ')
...
def extractPanel_news(dset, pset):
    '''
    ...
    pset['codeh'].insert(0, f"{dset['intend']}hdashboard.addPanel(\n" \
                          + f"{dset['intend']}    PanelNews(title='{pset['title']}', ")
    # optional properties
